        ...
        with open(self.filepath, 'r', newline='', encoding='utf-8') as csvfile, temp_file:
            ...
            for row in reader:
                if row['id'] == case.id:
                    ...
                    writer.writerow({'id': case.id,'location': case.location, "subject": case.subject, "description": case.description, 'priority': case.priority, 'repeated': case.repeated, 'real_est_id': row['real_est_id'], 'emp_id': row['emp_id'], 'date': row['date'], case.status: row['status'] })
                else:
                    row = {'id': row['id'],'location': row["location"], 'subject': row['subject'], 'description': row['description'], 'priority': row['priority'], 'repeated': row['repeated'], 'real_est_id': row['real_est_id'], 'emp_id': row['emp_id'], 'date': row['date'], case.status: row['status']}
                    writer.writerow(row)
